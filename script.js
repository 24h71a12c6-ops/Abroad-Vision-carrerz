...
document.addEventListener("DOMContentLoaded", function () {
    ...
    (function initProfileUI() {
        ...
        const pickSavedSummary = () => {
            ...
            const preferredNextKeys = ['country', 'destination', 'visaType', 'course', 'intake', 'highestQualification'];
            const nextPicked = {};
    ...
    if (regSection && regOverlay) {
        ...
        // Open after 4 seconds on page load.
        ...
        if (window.location.hash === '#registration-section') {
            ...
            // Don't auto-popup for users who are already signed in.
            // If the user explicitly logged out, show login immediately on next open.
            const signedIn = isRegisteredUser();
            const wantsLogin = localStorage.getItem('showLoginAfterLogout') === '1';
...
// Auth mode selection:
// - Default to Sign up
// - Show Log in only after explicit logout + reopening
const showLoginAfterLogout = localStorage.getItem('showLoginAfterLogout') === '1';
const signedInEmail = (localStorage.getItem('userEmail') || '').trim();
const lastUserEmail = (localStorage.getItem('lastUserEmail') || '').trim();

if (!signedInEmail && showLoginAfterLogout) {
    setAuthMode('login');
    const loginEmailInput = document.getElementById('loginEmail');
    if (loginEmailInput && lastUserEmail) loginEmailInput.value = lastUserEmail;
} else {
    setAuthMode('signup');
}
...
if (registrationForm) {
    ...
    registrationForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const response = await fetch(editMode ? 'http://localhost:5000/api/update-registration' : 'http://localhost:5000/api/register', {
